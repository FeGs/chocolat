...
RSpec.describe Services::CreateEvent do
  ...
  it 'creates an event in project id specific database, collection' do
    ...
    expect(connection.database_names).not_to include project.id.to_s

    result = Services::CreateEvent.new(project, event_name, data).execute
    expect(result.success?).to be true
    expect(event_count).to eq(1)
    expect(connection.database_names).to include project.id.to_s
  end
  ...
  it 'can create multiple events' do
    ...
    result = Services::CreateEvent.new(project, event_name, data_array).execute
    expect(result.success?).to be true
  ...
  it 'returns ResultError on invalid encoded data' do
    ...
    result = Services::CreateEvent.new(project, event_name, invalid_data).execute
